...
> **`/plugin` はデスクトップアプリでは使えない**（「一部のコマンドは Claude Code のターミナルでのみ使用できます」と出る）。デスクトップアプリで更新するときは、**設定** → **プラグイン** → 右上の **参照** → **コード** タブ → `yto-skills` の横の **…** → **アップデートを確認**。入れたあと自動では追随しないので、たまに確認する。
