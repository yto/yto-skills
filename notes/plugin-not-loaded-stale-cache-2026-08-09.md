# プラグインを入れたのにスキルが出てこない（古いキャッシュが原因）

- **作成日**: 2026-08-09
- **目的**: このリポのプラグインをインストール・有効化しているのに Claude Code のスキル一覧に出てこない、という症状に遭遇したときのために、原因と切り分け手順を残す。`claude plugin validate` も `claude plugin details` も「正常」と答えるので気づきにくい。
...
- 出典は無し。すべて手元（macOS / Claude Code 2.1.226）での実験と、`~/.claude/plugins/` 以下の実物の観察による。挙動は Claude Code のバージョンで変わりうる。
